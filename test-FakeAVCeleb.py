...
def resolve_fakeavceleb_paths(opt):
    ...
    if not dataset_root:
        raise ValueError("dataset_root is required. Provide --dataset_root or --manifest_csv.")
    if not manifest_csv:
        raise FileNotFoundError(
            "manifest_csv not found. Provide --manifest_csv, or ensure "
...
def export_error_reports(output_dir, report_prefix, records, threshold, save_all_predictions=False):
    ...
    if save_all_predictions:
        with open(pred_tsv_path, "w", encoding="utf-8") as f:
            f.write(
                "image_path\tsample_id\tlabel_text\tfake_type\trace\tgender\t"
                "true_label\tpred_label\tprob_fake\tthreshold\terror_type\n"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Defaults to <dataset_root>/test_manifest_with_status.csv.",
    )
