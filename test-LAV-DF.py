...
def resolve_lavdf2_paths(opt):
    real_path = str(getattr(opt, "real_list_path", "") or "").strip()
    fake_path = str(getattr(opt, "fake_list_path", "") or "").strip()
    lavdf2_root = str(getattr(opt, "lavdf2_root", "") or "").strip()

    if lavdf2_root:
        root = os.path.abspath(os.path.normpath(lavdf2_root))
        split_root = root
        if os.path.isdir(os.path.join(root, "test")):
            split_root = os.path.join(root, "test")

        auto_real = os.path.join(split_root, "0_real")
        auto_fake = os.path.join(split_root, "1_fake")
    ...
    if not real_path or not fake_path:
        raise ValueError(
            "real/fake path is required. Provide --real_list_path and --fake_list_path, "
            "or provide --lavdf2_root."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to LAV-DF-window or LAV-DF-window/test. "
             "If set, auto uses <root>/test/0_real and <root>/test/1_fake.",
    )
    ...
    # 解析 LAV-DF2 路径
    opt.real_list_path, opt.fake_list_path = resolve_lavdf2_paths(opt)
    print(f"[Info] real_list_path: {normalize_path_text(opt.real_list_path)}")
    print(f"[Info] fake_list_path: {normalize_path_text(opt.fake_list_path)}")

    # LAV-DF2 常见子目录计数（若存在）
    real_total = count_images_recursive(opt.real_list_path)
    fake_total = count_images_recursive(opt.fake_list_path)
    print(f"[Info] images(real/fake): {real_total}/{fake_total}")

    sub_real_from_real = os.path.join(opt.real_list_path, "from_real_video")
    sub_real_from_fake = os.path.join(opt.real_list_path, "from_fake_video_clean")
    sub_fake_tampered = os.path.join(opt.fake_list_path, "from_fake_video_tampered")
    if os.path.isdir(sub_real_from_real) or os.path.isdir(sub_real_from_fake) or os.path.isdir(sub_fake_tampered):
        cnt_rr = count_images_recursive(sub_real_from_real) if os.path.isdir(sub_real_from_real) else 0
        cnt_rf = count_images_recursive(sub_real_from_fake) if os.path.isdir(sub_real_from_fake) else 0
        cnt_ff = count_images_recursive(sub_fake_tampered) if os.path.isdir(sub_fake_tampered) else 0
        print(f"[Info] LAV-DF2 window sources: from_real_video={cnt_rr}, from_fake_video_clean={cnt_rf}, from_fake_video_tampered={cnt_ff}")
