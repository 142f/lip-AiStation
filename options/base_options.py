...
class BaseOptions:
    ...
    def initialize(self, parser):      
        ...
        parser.add_argument(
            "--strict_determinism",
            action="store_true",
            help="Fail on known nondeterministic PyTorch operations; slower and intended for reproducibility audits.",
        )
        parser.add_argument(
            "--audit_sampler_order",
            action="store_true",
            help="Log deterministic sampler and batch-boundary SHA-256 manifests each epoch.",
        )
        ...
        parser.add_argument("--prefetch_factor", type=int, default=2, help="DataLoader prefetch factor (batches per worker).")
